				...
				@foreach(config('constants.roles_permisions') as $key => $permission_header)
					...
					@if(isset($permissions[$key]))
						@foreach($permissions[$key] as $key2 => $permission)
							<div class="row pb-1">
								<div class="form-group col-md-12 text-left">
									<input type="checkbox" name="permission_ids[]" value="{{ $permission['id'] }}" {{ in_array($permission['id'], old('permission_ids', $role->permissions->pluck('id')->toArray())) ? 'checked' : '' }} > {{ $permission['display_name'] }}
								</div>
							</div>
						@endforeach
					@endif
